...
addEvent(document, 'readystatechange', function() {
    ...
    function updateCart(){
        ...
        for (var i = 0; i < cart_items.length; i++) {
            ...
            var item = cart_item.getAttribute('data-id');
            var quantity = cart_item.getAttribute('data-quantity');
            var price = cart_item.getAttribute('data-price');
            //Se almacena en BD local
            if(localStorage){
                $(document).ready(function(){
                        // Retrieve data
                        console.log(cart_item);
                        var key1 = 'cart_item' + i;
                        var key2 = 'quantity' + i;
                        var key3 = 'price' + i;
                        var totalv = price * quantity;
                        localStorage.setItem(key1, item);
                        localStorage.setItem(key2, quantity);
                        localStorage.setItem(key3, totalv);
                        //$('#cart_item').html(localStorage.getItem("cart_item"));
                        //$('#mailusuario').attr('href','mailto:' + localStorage.getItem("correo"));
                        //obtenerLocalizacion();
                });
            } else{
                alert("Lo Siento, Su Nevegador no Permite Almacenamiento Local.");
            }
